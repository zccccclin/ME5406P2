...
def make_env_subproc(train, num_env, args):
    def make_env(train):
        def _thunk():
            env = ReacherEnv(render=args.render, moving_goal=args.moving_goal, random_start=args.random_start, train=not args.test, dist_tol=args.dist_tol, env_name=args.env, reward_type=args.reward_type)
            return env
        return _thunk

    return SubprocVecEnv([make_env(train)
...
def main():
    ...
    parser.add_argument('--env', help='env', default='reacher', type=str)
    ...
    if args.test:
        args.num_envs = 1
    env = make_env_subproc(train=not args.test, num_env=args.num_envs, args=args)
